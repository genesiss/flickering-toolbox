...
import pytest
import itertools
from src.flicker_core import calculate_frequencies, calculate_cycle_params, generate_frame_pattern

@pytest.mark.parametrize("refresh_rate,min_frames_per_cycle,n_stim", [(60, 3, 5), (60, 2, 10), (60, 7, 50), (90, 5, 10), (120, 2, 7), (120, 10, 111)])
...
def test_min_frames_per_cycle_must_be_greater_than_one():
    """Should raise AssertionError if min_frames_per_cycle <= 1."""
    with pytest.raises(AssertionError, match="min_frames_per_cycle must be bigger than one"):
...
def test_flicker_onoff_sequence(refresh_rate, freq, duty_cycle, duration_s):
    ...
    # generate_frame_pattern returns one cycle, so tile it to match total_frames
    pattern = generate_frame_pattern(frames_per_cycle, on_frames)
    actual = [pattern[i % len(pattern)] for i in range(total_frames)]
